...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    {			
		...
		if(get_robot_remain_HP() == 0)    //阵亡清空标志位，防止复活后状态混乱
		{
			chassis_move_mode->gyroscope_flag = 0;  
			chassis_move_mode->auto_flag = 0;		
		}
    ...
    else if (chassis_behaviour_mode == CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW) //遥杆在上-遥控控制，遥杆在中-键鼠控制
    {
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
	...
	//这里没有设置小陀螺转速，设置在task的loop中
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector); //第三个参数获取键鼠，遥控的控制值
	//设置小陀螺旋转速度，对抗赛过检录要求正反小陀螺
