...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	if(hcan->Instance == CAN1)
	{
		switch (rx_header.StdId)
		{
			...
			case SUPER_CAP_ID:
			{
				CAP_CAN_RxCallback(&rx_header, &CAP_CANData, rx_data);
			}			
			default: break;    
		}					
	}
...
//射击3508发送电流
void CAN_cmd_shoot(int16_t right, int16_t left)
{
    uint32_t send_mail_box;
...
/**
* @brief          发送超级电容设定功率
* @param[in]      set_power:目标功率
  * @retval         none
  */
void CAN_cmd_SUPERCAP (uint16_t set_power)
{
	uint32_t send_mail_box;
	uint8_t sendbuf[8];
	supercap_tx_message.StdId = 0x210; //超级电容发送标识符
	supercap_tx_message.IDE = CAN_ID_STD;
	supercap_tx_message.RTR = CAN_RTR_DATA;
	supercap_tx_message.DLC = 0x08;
	sendbuf[0] = set_power >>8;
	sendbuf[1] = set_power;
	HAL_CAN_AddTxMessage(&hcan1, &supercap_tx_message, sendbuf, &send_mail_box);	
}
...
/**
  ...
  * @param[in]      none
  * @retval         超级电容数据指针
	*	旧版，不再使用@date 2023/4/19  
	*
	*/
const supercap_data_t *get_supercap_data_point(void)
{
	return &super_cap.supercap_data;
}
...
/**
  ...
  * @retval         射击数据指针
  */
const motor_measure_t *get_shoot_data_point1(void)
{
	return &motor_shoot[0];
}

const motor_measure_t *get_shoot_data_point2(void)
{
