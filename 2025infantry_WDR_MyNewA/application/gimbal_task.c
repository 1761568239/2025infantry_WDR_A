    ...
    {	
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            else
            {
				...
				CAN_cmd_shoot(get_shoot_control_point()->shoot_current_l, get_shoot_control_point()->shoot_current_r);
            }
        }		
...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    //电机数据指针获取
    init->gimbal_yaw_motor.gimbal_motor_measure = get_yaw_gimbal_motor_measure_point();    //yaw轴电机数据指针
    init->gimbal_pitch_motor.gimbal_motor_measure = get_pitch_gimbal_motor_measure_point(); //pitch轴电机数据指针
    //陀螺仪数据指针获取
    init->gimbal_INT_angle_point = get_INS_angle_point();     //获取欧拉角, 0:yaw, 1:pitch, 2:roll 单位 rad
    init->gimbal_INT_gyro_point = get_gyro_data_point();      //获取角速度,0:x轴, 1:y轴, 2:roll轴 单位 rad/s
    //遥控器数据指针获取
	init->gimbal_rc_ctrl = get_remote_control_point();		 //遥控器数据指针获取	
    /*初始化本次和上次均为原始电机数据控制式*/
	...
	//自瞄和小陀螺切换相关
	init->auto_gyro_mode = GIMBAL_UP_AUTO;       //上电默认为自瞄模式，可更改
	init->gyro_change_channel = 0;               //初始化这一次滚轮状态为0
	init->last_gyro_change_channel =0;           //初始化上一次滚轮状态为0
}
...
/**
  * @brief      云台控制过程中对角度目标进行限制             
  * @param[in]  gimbal_motor:云台电机指针
  * @param[in]  add:云台目标增加值
  * @retval    无       
  */
static void gimbal_absolute_angle_limit_pitch(gimbal_motor_t *gimbal_motor, fp32 add)
{
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
    ...
    gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, \
...
/**
  * @brief    云台电机PID计算函数，由于云台使用绝对角度控制，需要单独对ERROR做处理，且微分项是
			  直接使用陀螺仪角速度值，故单独进行PID计算函数做定义---add by qiyin2025.3.29
  * @param[in]  pid: PID结构体指针
  * @param[in]  get: 当前值
  * @param[in]  set: 设置值
  * @param[in]  error_delta:微分项角数据
  * @retval           
  */
static fp32 gimbal_PID_calc(gimbal_PID_t *pid, fp32 get, fp32 set, fp32 error_delta)
{
